...
<html lang="es">
    <head>
        ...
        <style>
            ...
            .header .logo-text { /* Estilo para el texto del logo "Mantenimiento - Categorías" */
                font-size: 22px;
                color: var(--text);
            ...
            /* Estilos del CRUD de Categorías (originales de tu archivo - se mantienen) */
            .container-main {
    ...
    <body>
        ...
        <aside class="sidebar">
            ...
            <nav class="menu">
                ...
                <a href="ServletCategorias" class="menu__item active"> <!-- CATEGORIAS ACTIVO -->
        ...
        <main class="main-content">
            ...
            <div class="container">
                <div class="container-main">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        <h1>Listado de Categorías</h1> <!-- Cambiado "Elementos" a "Categorías" -->
                        <div> <!-- Contenedor para los botones -->
                            <c:if test="${rol == 'Admin'}">
                                <button type="button" class="btn btn-add me-2" data-bs-toggle="modal" data-bs-target="#categoriaModal"
                                        onclick="prepararModalAgregar()">
                                    <i class="bi bi-plus-circle"></i> Agregar Categoría <!-- Cambiado "Elemento" a "Categoría" -->
                                </button>
                                <!-- Nuevo botón para descargar el reporte -->
                                <a href="${pageContext.request.contextPath}/ServletReportes?nombre=categorias" class="btn btn-report" target="_blank">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover table-bordered table-custom" id="tablaCategorias"> <!-- Cambiado "tablaElementos" a "tablaCategorias" -->
                            <thead>
                                <tr>
                                    <th>ID</th>
                                    <th>Nombre</th>
                                ...
                                <c:choose>
                                    <c:when test="${not empty listaCategorias}">
                                        <c:forEach var="c" items="${listaCategorias}">
                                            <tr data-id="${c.codigoCategoria}"
                                                data-nombre="${c.nombreCategoria}"
                                                data-descripcion="${c.descripcionCategoria}">
                                                <td>${c.codigoCategoria}</td>
                                                <td>${c.nombreCategoria}</td>
                                                <td>${c.descripcionCategoria}</td>
                                                <td>
                                                    <button type="button" class="btn btn-edit btn-sm me-2" data-bs-toggle="modal" data-bs-target="#categoriaModal"
                                                            onclick="prepararModalEditar(this)">Editar</button>
                                                    <a href="${pageContext.request.contextPath}/ServletCategorias?accion=eliminar&id=${c.codigoCategoria}"
            ...
            <div class="modal fade" id="categoriaModal" tabindex="-1" aria-labelledby="categoriaModalLabel" aria-hidden="true"> <!-- Cambiado "elementoModal" a "categoriaModal" -->
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="categoriaModalLabel"></h5> <!-- Cambiado "elementoModalLabel" a "categoriaModalLabel" -->
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <form id="categoriaForm" action="${pageContext.request.contextPath}/ServletCategorias" method="post"> <!-- Cambiado "elementoForm" a "categoriaForm" -->
                                <input type="hidden" name="accion" id="formAccion" value="agregar">
                                <input type="hidden" name="id" id="formCodigoCategoria"> <!-- Cambiado "formIdElemento" a "formCodigoCategoria" -->
        ...
        <script>
            function prepararModalAgregar() {
                document.getElementById('categoriaModalLabel').innerText = 'Agregar Nueva Categoría'; // Cambiado "elementoModalLabel" a "categoriaModalLabel"
                document.getElementById('categoriaForm').reset(); // Cambiado "elementoForm" a "categoriaForm"
                document.getElementById('formAccion').value = 'agregar';
                document.getElementById('formCodigoCategoria').value = ''; // Cambiado "formIdElemento" a "formCodigoCategoria"
            }
            function prepararModalEditar(button) {
                ...
                document.getElementById('categoriaModalLabel').innerText = 'Editar Categoría ID: ' + id; // Cambiado "elementoModalLabel" a "categoriaModalLabel"
                document.getElementById('formCodigoCategoria').value = id; // Cambiado "formIdElemento" a "formCodigoCategoria"
