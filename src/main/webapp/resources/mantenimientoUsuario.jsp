<%-- 
    ...
    Created on : 30 jul 2025, 10:40:25
    Author     : informatica
--%>
            ...
            <div class="container-main">
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover table-bordered table-custom" id="tablaElementos">
                        ...
                        <tbody>
                            <%
                                List<Usuarios> listaUsuarios = (List<Usuarios>) request.getAttribute("listaCategorias");
                                if (listaUsuarios != null && !listaUsuarios.isEmpty()) {
                                    for (Usuarios c : listaUsuarios) {
                            %>
                            <tr data-id="<%= c.getCodigoUsuario()%>" 
                                data-nombre="<%= c.getNombreUsuario()%>" 
                                data-apellido="<%= c.getApellidoUsuario()%>"
                                data-telefono="<%= c.getTelefono()%>"
                                data-direccion="<%= c.getDireccionUsuario()%>"
                                data-email="<%= c.getEmail()%>"
                                data-contrasena="<%= c.getContrasena()%>"
                                data-estado="<%= c.getEstadoUsuario()%>"
                                data-rol="<%= c.getRol()%>"
                                data-nacimiento="<%= c.getFechaNacimiento()%>">
                                <td><%= c.getCodigoUsuario()%></td>
                                <td><%= c.getNombreUsuario()%></td>
                                <td><%= c.getApellidoUsuario()%></td>
                                <td><%= c.getTelefono()%></td>
                                <td><%= c.getDireccionUsuario()%></td>
                                <td><%= c.getEmail()%></td>
                                <td><%= c.getContrasena()%></td>
                                <td><%= c.getEstadoUsuario()%></td>
                                <td><%= c.getRol()%></td>
                                ...
                                <td>
                                    ...
                                    <a href="${pageContext.request.contextPath}/ServletCategorias?accion=eliminar&id=<%= c.getCodigoUsuario()%>" 
